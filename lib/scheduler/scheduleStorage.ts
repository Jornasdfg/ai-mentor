...
export async function readScheduleBlocks(): Promise<ScheduleBlock[]> {
  try {
    const raw = await fs.readFile(filePath("schedule_blocks.json"), "utf-8");
...
export async function readScheduleRuns(): Promise<ScheduleRun[]> {
  try {
    const raw = await fs.readFile(filePath("schedule_runs.json"), "utf-8");
    return JSON.parse(raw) as ScheduleRun[];
  } catch {
    return [];
  }
}
...
export async function readSchedulingWindows(): Promise<SchedulingWindow[]> {
  try {
    const raw = await fs.readFile(filePath("scheduling_windows.json"), "utf-8");
    return JSON.parse(raw) as SchedulingWindow[];
  } catch {
    return [];
  }
}
...
export async function ensureDefaultSchedulingWindows(): Promise<void> {
  ...
  const defaults: SchedulingWindow[] = [
    {
      id: "window_work",
      name: "Werk",
      type: "work",
  ...
  await writeSchedulingWindows(defaults);
}