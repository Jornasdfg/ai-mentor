    ...
    ">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-semibold text-zinc-600 uppercase tracking-wider">Google Calendar</span>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        <h2 className="text-sm font-semibold text-zinc-900 leading-snug">{event.title}</h2>

        <div className="space-y-2 text-xs">
          {isAllDay ? (
            <div className="flex gap-2">
              <span className="text-zinc-600 w-14 shrink-0">Dag</span>
              <span className="text-zinc-700">{localDateTime(event.start)}</span>
            </div>
          ) : (
            <>
              <div className="flex gap-2">
              ...
              <div className="flex gap-2">
                ...
                <span className="text-zinc-700">{localTime(event.end)}</span>
              </div>
              <div className="flex gap-2">
                <span className="text-zinc-600 w-14 shrink-0">Duur</span>
                <span className="text-zinc-700">{durationStr(event.start, event.end)}</span>
              </div>
            </>
          )}
        </div>
        ...
        {event.description && (
          <div className="bg-white/60 rounded-lg p-3">
            <p className="text-[10px] text-zinc-600 uppercase tracking-wider mb-1">Beschrijving</p>
            <p className="text-xs text-zinc-700 leading-relaxed whitespace-pre-wrap">{event.description}</p>
          </div>
        )}
      ...
      <div className="px-4 py-3 border-t border-gray-200 space-y-2 shrink-0">
        ...
        {onImportAsTask && (
          <button
            ...
            className="w-full py-2 text-xs font-medium rounded-lg border border-gray-200 text-zinc-600 hover:text-zinc-800 hover:border-gray-300 transition-colors"
          >
            + Importeer als taak
          </button>
        )}
