...
export default function MentorChat({ onComplete }: { onComplete?: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full bg-transparent min-w-0">
      {/* Header */}
      <div className="px-4 py-2.5 border-b border-gray-200/70 bg-white/70 backdrop-blur-sm shrink-0 flex items-center justify-between gap-2 min-w-0">
        <span className="text-xs font-extrabold text-zinc-700 tracking-wide truncate">AI Mentor</span>
        <span className="text-[10px] text-zinc-500 shrink-0">gpt-4o-mini</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden p-4 space-y-3 min-h-0">
            ...
            <div className="grid grid-cols-1 gap-1.5">
              {QUICK_STARTERS.map(s => (
                <button
                  ...
                  className="text-left text-xs text-zinc-700 bg-white border border-gray-200/80 rounded-2xl px-3.5 py-2.5 shadow-card hover:shadow-soft hover:-translate-y-0.5 hover:text-accent active:scale-[0.98] transition-all"
                >
                  {s}
                </button>
              ))}
        ...
        {messages.map(msg => (
          <div key={msg.id} className={`flex w-full animate-msg-in ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
            <div className="max-w-[85%] min-w-0 space-y-2">
              {/* Bubble */}
              <div className={`rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed whitespace-pre-wrap break-anywhere shadow-soft ${
                msg.role === "user"
                  ? "bg-gradient-to-br from-accent to-accent2 text-white rounded-br-md"
                  : "bg-white text-zinc-900 rounded-bl-md"
              }`}>
                {msg.content}
              </div>
        ...
        {recording ? (
          ...
          <div className="flex gap-2 items-end">
            ...
            <button
              ...
              className="w-11 h-11 sm:w-10 sm:h-10 flex items-center justify-center rounded-full bg-gradient-to-br from-accent to-accent2 text-white shadow-soft hover:brightness-105 active:scale-90 disabled:opacity-40 disabled:cursor-not-allowed transition-all shrink-0 text-lg"
            >
              ↑
            </button>
          </div>
